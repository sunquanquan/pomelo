...
var DEFAULT_SECRET = 'pomelo_session_secret_yztwy';
...
pro.auth = function(token, cb) {
	...
	var code = tokenService.create('bb22c0f85c0c476ba26ee5dac9f2b745',date,'pomelo_session_secret_yztwy');
	console.log(code);*/
	var res = tokenService.parse(token, this.secret);
	...
	userDao.getUserById(res.gameID, function(err, user) {
		if(err) {
			cb(err);
			return;
		}
